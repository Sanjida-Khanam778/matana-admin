...
function HouseIcon() {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#92713a" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
function VideoModal({ business, onClose }) {
    return (
        <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-[#1a1a1a] rounded-2xl overflow-hidden w-full max-w-xl shadow-2xl" onClick={e => e.stopPropagation()}>
                <div className="flex items-start justify-between px-5 py-4">
                    <div>
                        <p className="text-white text-sm font-semibold">{business.videoTitle}</p>
                ...
                <video src={business.videoSrc} controls autoPlay className="w-full" style={{ maxHeight: "340px" }} />
            </div>
        </div>
    );
}
...
function SlideCard({ business, onPlay }) {
    return (
        <div className="relative rounded-2xl overflow-hidden flex-shrink-0 w-full h-[510px] select-none font-inter">
            <img src={business.image} alt={business.name} className="absolute inset-0 w-full h-full object-cover pointer-events-none" draggable={false} />
            ...
            >
                <div className="w-20 h-20 rounded-full bg-white/90 flex items-center justify-center shadow-lg hover:scale-105 transition-transform">
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="#1a5c3a">
            ...
            <div className="absolute bottom-0 left-10 p-7 z-10">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <p className="text-white text-3xl font-semibold leading-tight drop-shadow">{business.name}</p>
                        <p className="text-white/75 text-sm leading-tight">{business.tagline}</p>
                    </div>
                </div>
                <div className="flex gap-2 mb-6 flex-wrap">
                    {business.badges.map((b, i) => (
                        <span key={i} className={`px-4 py-2 rounded-full text-sm font-medium ${i === 0 ? "bg-primary text-white" : "bg-white/20 text-white backdrop-blur-sm"}`}>
                            {b}
                        </span>
                    ))}
                </div>
    ...
    return (
        <section className="w-full bg-white py-20">
            ...
            <div className="text-center mb-10">
                ...
                <h2 className="text-4xl font-bold text-gray-900">Featured Businesses</h2>
            </div>
            ...
            >
                <div
                    ...
                    style={{
                        ...
                        transition: isSnapping ? "transform 1.85s cubic-bezier(0.25, 1, 0.5, 1)" : "none",
            ...
            <div className="flex items-center justify-center gap-2 mt-8">
                {Array.from({ length: DOT_COUNT }).map((_, i) => (
                    <button
                        ...
                        className={`transition-all duration-300 rounded-full ${activeDot === i ? "w-8 h-2.5 bg-[#1a5c3a]" : "w-2.5 h-2.5 bg-gray-300 hover:bg-gray-400"
                            }`}
                    />
                ))}
            </div>
            ...
            {/* Video Modal */}
            {playingVideo && (
                <VideoModal business={playingVideo} onClose={() => setPlayingVideo(null)} />
            )}
        </section>
    );
}